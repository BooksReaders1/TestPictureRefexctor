...
- Q: 关于错误处理机制，采用哪种策略？ → A: 自动重试（有最大次数限制）+ 错误添加重试按钮支持手动重试
- Q: 关于图片质量和性能的权衡，优先选择？ → A: B - 优先保证性能，允许适当降低图片质量
- Q: 关于状态管理，采用哪种方案？ → A: B - 使用Vuex进行状态管理，集中管理应用状态
- Q: 关于可观察性设计，需要监控哪些关键指标？ → A: B - 监控API响应时间和错误率，设置性能告警阈值
- Q: 关于API域名管理，采用哪种策略？ → A: 使用动态获取域名，并使用固定域名进行降级处理
